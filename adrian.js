...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const isCreator = [botNumber, ...global.owner, '6281212035575@s.whatsapp.net'].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
newReply(`*「  ᴋᴏɴᴇᴋᴏ-ᴍᴅ ꜱᴄʀɪᴘᴛ  」*\n\n*Instagram:*\n*https://instagram.com/ravaja_*\n\n\n *Jangan lupa untuk donate agar bot bisa update terus!* \n\n*Link* :\n\nhttps://saweria.co/Rava  `)
        break
...
showAdAttribution: false,
		...
		case 'quotes':
			...
			newReply(`_${data.result.quote}_\n\n*― ${data.result.by}*`)
			break
		case 'quotesanime':
		...
		case 'bucin':
			...
			break
		case 'fact':	
			var { data } = await axios.get(`https://api.lolhuman.xyz/api/random/faktaunik?apikey=22253ba266448d1097bef962`)
			newReply(data.result)
			break
		case 'randomnama':
...
( Q.S ${res.result.data.surah.name.transliteration.id} : ${res.result.data.number.inSurah} )`
			...
			if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
                db.data.users[m.sender].limit -= 5
			newReply(mess.wait)
			axios.get(`https://api.lolhuman.xyz/api/ytsearch?apikey=${apikey}&query=${full_args}`)
				.then(({ data }) => {
			...
			if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
                db.data.users[m.sender].limit -= 5
			newReply(`Sedang mencari *${text}* ! Mohon tunggu`)
			axios.get(`https://api.lolhuman.xyz/api/ytplay?apikey=22253ba266448d1097bef962&query=${text}`)
				.then(({ data }) => {
					var caption = `❖ Title     :*${data.result.title}*\n`
					caption += `❖ Channel     :*${data.result.uploader}*\n`
					caption += `❖ Durasi     :*${data.result.duration}*\n`
					caption += `❖ Description     :${data.result.description}`
					conn.sendMessage(m.chat, { image: { url: data.result.thumbnail }, caption }).then(() => {
						conn.sendMessage(m.chat, { audio: { url: data.result.audio.link }, mimetype: 'audio/mp4', fileName : `${data.result.title}.mp3` })
			...
			if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
                db.data.users[m.sender].limit -= 5
			if (args.length == 0) return newReply(`Example: ${prefix + command} Melukis Senja`)
			axios.get(`https://api.lolhuman.xyz/api/jooxplay?apikey=22253ba266448d1097bef962&query=${full_args}`).then(({ data }) => {
				...
				conn.sendMessage(m.chat, { image: { url: data.result.image }, caption }).then(() => {
					conn.sendMessage(m.chat, { audio: { url: data.result.audio[0].link }, mimetype: 'audio/mp4', fileName: `${data.result.title}.mp3` })
				})
			})*/
			break
			case 'ytmp3':
...
break
		...
		/*case 'qrreader':
            ...
            newReply(`Please Wait (this took a few second)`)
			if (!quoted) return (`Balas Video/Image Dengan Caption ${prefix + command}`)
				if (isMedia) {
                    ...
                    await 
                    axios.get(`https://api.lolhuman.xyz/api/read-qr?apikey=22253ba266448d1097bef962&img=${anu}`).then(({ data }) =>
				newReply(data.result)
                } else {
		...
		case 'brainly':{
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} Melukis Senja`)
			newReply(mess.wait)
			var { data } = await axios.get(`https://api.lolhuman.xyz/api/lirik?apikey=${apikey}&query=${text}`)
			newReply(data.result)
			break
			...
			// GAME
                ...
                break
				case 'simi': 
				if (args.length == 0) return newReply(`Example: ${prefix + command} Halo`)
				axios.get(`https://api.lolhuman.xyz/api/simi?apikey=22253ba266448d1097bef962&text=${full_args}&badword=true`).then(({ data }) => {
				newReply(data.result)
				})
				break
			...
			//https://api.lolhuman.xyz/api/dall-e?apikey=22253ba266448d1097bef962&text=
			// AI MENU
			...
			//axios.get(`https://xzn.wtf/api/openai?text=${encodeURIComponent(full_args)}&apikey=ravaja`).then(({ data }) => {
				newReply(data.result)
			})
			break
			case 'txt2img':
			if (args.length == 0) return newReply(`Example: ${prefix + command} tempe goreng`)
			newReply(mess.wait)
			 conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/dall-e?apikey=22253ba266448d1097bef962&text=${full_args}` }, caption: `Sukses` }, { quoted: m })
			break
			case 'diffusion':
			if (args.length == 0) return newReply(`Example: ${prefix + command} a portrait of an old monk,highly detailed`)
			newReply(mess.wait)
			 conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/diffusion-prompt?apikey=22253ba266448d1097bef962&prompt=${full_args}` }, caption: `Sukses` }, { quoted: m })
			break
            ...
            newReply(`Please Wait (this took a few second)`)
                ...
                if (isMedia) {
                    ...
                    const anu = await TelegraPh(media)
                    await 
                    //conn.sendMessage(m.chat, { image: { url: `https://api.itsrose.life/image/unblur?url=${anu}&apikey=010372fd654ab6ae27af5e7a` }, caption: `Sukses membuat hd` }, { quoted: m })
                    conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/upscale?apikey=22253ba266448d1097bef962&img=${anu}` }, caption: `Sukses membuat hd` }, { quoted: m })
                } else {
                newReply('Reply gambar nya bang')
                }
            }
            ...
            newReply(`Please Wait (this took a few second)`)
                ...
                if (isMedia) {
                    ...
                    await 
                    //conn.sendMessage(m.chat, { image: { url: `https://api.itsrose.life/image/unblur?url=${anu}&apikey=010372fd654ab6ae27af5e7a` }, caption: `Sukses membuat hd` }, { quoted: m })
                    conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/imagetoanime?apikey=22253ba266448d1097bef962&img=${anu}` }, caption: `Follow Ig Gwejh donk \nLink : instagram.com/ravaja_` }, { quoted: m })
                } else {
                newReply('Reply gambar nya bang')
                }
            }
            ...
            break
			case 'telesticker':
				if (args.length == 0) return newReply(`Example: ${prefix + command} https://t.me/addstickers/LINE_Menhera_chan_ENG`)
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} dryan.pu`)
			newReply(mess.wait)
			axios.get(`https://api.lolhuman.xyz/api/osuname/${text}?apikey=22253ba266448d1097bef962`).then(({ data }) => {
				var caption = `UserId : ${data.result.user_id}\n`
				caption += `UserName : ${data.result.username}\n`
				caption += `Join dari : ${data.result.join_date}\n`
				caption += `Count300 : ${data.result.count300}\n`
				caption += `Count100 : ${data.result.count100}\n`
				caption += `count50 : ${data.result.count50}\n`
				caption += `PlayCount : ${data.result.playcount}\n`
				caption += `Ranked Score : ${data.result.ranked_score}\n`
				caption += `Total Score : ${data.result.total_score}\n`
				caption += `pp rank : ${data.result.pp_rank}\n`
				caption += `Level : ${data.result.level}\n`
				caption += `ppRaw : ${data.result.pp_raw}\n`
				caption += `Akurasi : ${data.result.accuracy}%\n`
				caption += `Rank SSH : ${data.result.count_rank_ssh}\n`
				caption += `Rank SS : ${data.result.count_rank_ss}\n`
				caption += `Rank SH : ${data.result.count_rank_sh}\n`
				caption += `Rank S : ${data.result.count_rank_s}\n`
				caption += `Rank A : ${data.result.count_rank_a}\n`
				caption += `Negara : ${data.result.country}\n`
				caption += `Total Bermain PerDetik : ${data.result.total_seconds_played}\n`
				caption += `pp country : ${data.result.pp_country_rank}\n`
				newReply(caption)
			})
			...
			case 'growtopia': {
			newReply(mess.wait)
			axios.get(`https://api.lolhuman.xyz/api/growtopia?apikey=22253ba266448d1097bef962`).then(({ data }) => {
				var caption = `Player Online : *${data.result.player_online}*\n`
				caption += `WOTD : *${data.result.wotd.name}*`
				conn.sendMessage(m.chat, { image: { url: data.result.wotd.preview }, caption })
			})
			...
			case 'ssweb':
                  ...
                  newReply(mess.wait)
                  wm_dryan = args[0]
                  ini_buffer = await getBuffer(`https://api.lolhuman.xyz/api/ssweb?apikey=22253ba266448d1097bef962&url=${text}`)
        ...
        break
			...
			newReply(mess.wait)
				...
				until = get_args[14]
				img = get_args[15]
			    titidnya = `Selamat yaa ktp anda selesai!`
            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/ktpmaker?apikey=22253ba266448d1097bef962&nik=${nik}&prov=${prov}&kabu=${kabu}&name=${name}&ttl=${ttl}&jk=${jk}&jl=${jl}&rtrw=${rtrw}&lurah=${lurah}&camat=${camat}&agama=${agama}&nikah=${nikah}&kerja=${kerja}&warga=${warga}&until=${until}&img=${img}`}, caption: titidnya}, {quoted: m})
			break*/
				//https://api.lolhuman.xyz/api/ktpmaker?apikey=22253ba266448d1097bef962&nik=456127893132123&prov=bumipertiwi&kabu=fatamorgana&name=LoL%20Human&ttl=mars,%2099-99-9999&jk=belum%20ditemukan&jl=jl%20wardoyo&rtrw=999/999&lurah=turese&camat=imtuni&agama=alhamdulillah%20islam&nikah=jomblo%20kack&kerja=mikirin%20dia&warga=indo%20ori%20no%20kw&until=hari%20kiamat&img=https://i.ibb.co/Xb2pZ88/test.jpg
	        case 'carbon':
    ...
    // Text Prome
        ...
        break
			...
			if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit)
            db.data.users[m.sender].limit -= 2
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/random/bts?apikey=22253ba266448d1097bef962` } })
			break
			case 'logo3':
			if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit)
            db.data.users[m.sender].limit -= 2
...
conn.sendMessage(m.chat, {
    ...
    contextInfo: {
      externalAdReply: {
        ...
        sourceUrl: "https://chat.whatsapp.com/GFtm25cVEmC1ZtqoCenClf",
        mediaType: 1,
...
let e = String(err)
conn.sendMessage("6281212035575@s.whatsapp.net", { text: "👋🏻 Hello developer, ada yang error nih! di bagian " + util.format(e), 
contextInfo:{
